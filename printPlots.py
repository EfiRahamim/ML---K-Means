...
z = np.random.rand(2, 3)  # load centroids
...
# initialize an array to save all the squar distances of each centroid from it's pixels
...
for i in range(20):
    ...
    # initialize distance array to sum up all the distances from centroid to it's pixels
    cent_distance_sum = np.array([0.] * cent_num)
    ...
    for pix_row in range(pix_num):
        ...
        cent_distance_sum[closest_cent] += pow(min_distance, 2)
    ...
    for k in range(cent_num):
        ...
        if cent_counter[k] != 0:
            ...
            # calculate the average distance of each centorid
            cent_distance_sum[k] = cent_distance_sum[k] / cent_counter[k]

    average_loss = numpy.append(average_loss, sum(cent_distance_sum))
    iterations = numpy.append(iterations, i)
...
plt.plot(iterations, average_loss)
plt.xlabel("Iteration")
plt.ylabel("Average loss/cost value")
plt.title("K = 2")
plt.show()
# print(average_loss)
# print(iterations)
